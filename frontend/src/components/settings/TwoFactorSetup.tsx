...
export default function TwoFactorSetup() {
  ...
  if (loading && mode === 'idle') {
    return <div className="text-sm text-white/40">Загрузка...</div>
  }
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="text-sm font-medium text-white/80">Двухфакторная аутентификация (TOTP)</div>
      ...
      {mode === 'setup' && (
        <div className="mt-4 p-4 rounded-xl border border-white/10 bg-white/5 space-y-4">
          {setupStep === 'scan' && (
            <>
              <div className="text-sm font-medium text-white/80">Шаг 1: Отсканируйте QR-код</div>
              <div className="flex flex-col items-center gap-4">
                {qrImageUrl && (
                  <img src={qrImageUrl} alt="QR code" className="rounded-lg" width={180} height={180} />
                )}
                <div className="text-center">
                  <div className="text-xs text-white/45 mb-1">или введите секрет вручную:</div>
          ...
          {setupStep === 'confirm' && (
            <>
              <div className="text-sm font-medium text-white/80">Шаг 2: Подтвердите код</div>
              <input
                type="text"
                inputMode="numeric"
                value={code}
                ...
                placeholder="000000"
                className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/15 text-white text-center text-xl tracking-widest font-mono focus:outline-none focus:border-[var(--accent)]"
                maxLength={6}
                autoFocus
              />
              <div className="flex gap-2">
                <button
                  ...
                  className="flex-1 py-2 rounded-lg border border-white/15 text-white/60 text-sm"
                >
                  ← Назад
                </button>
                <button
      ...
      {mode === 'disable' && (
        <div className="mt-4 p-4 rounded-xl border border-red-500/20 bg-red-950/10 space-y-3">
          ...
          <div className="text-xs text-white/50">Введите пароль для подтверждения</div>
          <input
            type="password"
            value={disablePassword}
            onChange={e => setDisablePassword(e.target.value)}
            placeholder="Ваш пароль"
            className="w-full px-3 py-2 rounded-lg bg-white/5 border border-white/15 text-white focus:outline-none focus:border-red-500/50"
            autoFocus
          />
          <div className="flex gap-2">
            <button
