          ...
          >
            <div style={{ padding: "10px 0" }}>
              {logs.length > 0 ? logs.map((line, i) => {
                ...
                // Format B1: "...python[PID]: date HH:MM:SS | LEVEL | location | message"
                const mB1 = line.match(/^\d{4}-\d{2}-\d{2}T[^\s]*\s+\S+\s+\S+:\s*\d{4}-\d{2}-\d{2}\s+([\d:]{8})\s*\|\s*(WARNING|ERROR|CRITICAL|INFO|DEBUG)\s*\|\s*[^|]*\|\s*(.*)$/)
                // Format B2: "...python[PID]: date HH:MM:SS | Module | LEVEL | location | message"
                const mB2 = !mB1 ? line.match(/^\d{4}-\d{2}-\d{2}T[^\s]*\s+\S+\s+\S+:\s*\d{4}-\d{2}-\d{2}\s+([\d:]{8})\s*\|\s*[^|]*\|\s*(WARNING|ERROR|CRITICAL|INFO|DEBUG)\s*\|\s*[^|]*\|\s*(.*)$/) : null
                const mB = mB1 || mB2
                ...
                if (!parsed) {
                  return (
                    <div key={i} style={{ display: "flex", alignItems: "baseline", background: bgColor }}>
                      ...
                      <span style={{ color: isError ? "#f87171" : "#4a5568", fontSize: 12, lineHeight: 1.65, whiteSpace: "pre-wrap", wordBreak: "break-all", flex: 1, paddingRight: 14 }}>
                        {line}
                      </span>
                    </div>
                  )
