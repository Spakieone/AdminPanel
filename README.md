...
Если на сервере включён UFW, Docker-контейнер не сможет подключиться к бот-API на хосте. Нужно разрешить трафик из Docker-сети:
...
> **Без этого правила** панель не сможет связаться с ботом — статус бот-API будет «Не работает» (ConnectTimeout).
