...
## Установка вместе с ботом

Если панель и бот стоят на одном сервере, нужно настроить API-модуль бота:

**1. Адрес прослушивания** — в файле `/root/bot/modules/api/settings.py`:

```python
HTTP_API_HOST = "0.0.0.0"    # НЕ "127.0.0.1" — иначе Docker-контейнер не достучится
HTTP_API_PORT = 7777
```

**2. Firewall (UFW)** — если включён, разрешить трафик из Docker-сети:

```bash
ufw allow from 172.16.0.0/12 to any port 7777 comment "AdminPanel Docker -> Bot API"
```

**3. Профиль бота** — в панели указать адрес `127.0.0.1:7777` (панель автоматически заменит на `host.docker.internal` внутри контейнера).

> **Без этих настроек** статус бот-API будет «Не работает» (ConnectTimeout).
